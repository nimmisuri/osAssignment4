...
op := devctl(dc$discread, 1, window_left, window);
if op < 0 then
returnto(command_loop, op);
op := devctl(dc$discread, 1, window_right, window+128);
if op < 0 then
returnto(command_loop, op);
write(tty, "\n");
...
let get_free_block() be {
let b = superblock ! sb_firstfreeblock;
if b >= superblock ! free.blocks then {
error_reason := "disc is full";
resultis err$software }
superblock ! sb_firstfreeblock +:= 1;
window.pointer +:= 1;
if window.pointer > 255 then
shift_window_right();

resultis b }
let make_block_free(b) be {
if b < superblock ! list.last + 2 then {
error_reason := "all usuable blocks are free";
resultis err$software }
superblock ! sb_firstfreeblock -:= 1;
window.pointer -:= 1;
...
let op, blocknum, enter_entry, level;
let hb = vec(128);
...
enter_entry := enter_into_root_directory(fname);
